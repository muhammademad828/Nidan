...
@section('page-title', 'Edit Event Slide')
...
<div class="max-w-4xl">
    <header class="mb-8">
        ...
        <h1 class="text-3xl font-serif text-nidan-text">Edit Event Slide</h1>
    </header>

    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        <form action="{{ route('admin.event-slides.update', $eventSlide) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title_en" value="{{ old('title_en', $eventSlide->title_en) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="subtitle_en" value="{{ old('subtitle_en', $eventSlide->subtitle_en) }}"
                               class="w-full rounded-xl border-gray-200 focus:border-nidan-gold focus:ring-nidan-gold transition-all">
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div dir="rtl">
                        ...
                        <input type="text" name="title_ar" value="{{ old('title_ar', $eventSlide->title_ar) }}" required
                               class="w-full rounded-xl border-gray-200 focus:border-nidan-gold focus:ring-nidan-gold transition-all">
                    </div>
                    <div dir="rtl">
                        <label class="block text-sm font-medium text-gray-700 mb-2">العنوان الفرعي (AR)</label>
                        <input type="text" name="subtitle_ar" value="{{ old('subtitle_ar', $eventSlide->subtitle_ar) }}"
                               class="w-full rounded-xl border-gray-200 focus:border-nidan-gold focus:ring-nidan-gold transition-all">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6 mb-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <select name="media_type" class="w-full rounded-xl border-gray-200 focus:border-nidan-gold focus:ring-nidan-gold transition-all">
                            <option value="image" {{ $eventSlide->media_type === 'image' ? 'selected' : '' }}>Image</option>
                            <option value="video" {{ $eventSlide->media_type === 'video' ? 'selected' : '' }}>Video</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Sort Order</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', $eventSlide->sort_order) }}" required
                               class="w-full rounded-xl border-gray-200 focus:border-nidan-gold focus:ring-nidan-gold transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="mb-4">
                        @if($eventSlide->media_type === 'video')
                            <video class="w-full max-w-sm h-48 rounded-2xl object-cover shadow-inner bg-gray-900" controls>
                                <source src="{{ $eventSlide->media_url }}" type="video/mp4">
                            </video>
                        @else
                            <img src="{{ $eventSlide->media_url }}" alt="" class="w-full max-w-sm h-48 rounded-2xl object-cover shadow-md">
                        @endif
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        <div>
                            ...
                            <input type="text" name="media_url" value="{{ old('media_url', $eventSlide->media_url) }}"
                                   class="w-full rounded-xl border-gray-200 focus:border-nidan-gold focus:ring-nidan-gold text-sm transition-all">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Option 2: Replace File</label>
                ...
                <label class="flex items-center gap-3 cursor-pointer group">
                    <div class="relative">
                        <input type="checkbox" name="is_active" value="1" {{ $eventSlide->is_active ? 'checked' : '' }} class="sr-only peer">
